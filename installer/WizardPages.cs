...
public sealed partial class SetupWindow
{
    ...
    static BitmapSource GuideImage(string filename){using var stream=typeof(SetupWindow).Assembly.GetManifestResourceStream("CadToolkitSetup.images."+filename)??throw new InvalidDataException("缺少內建教學圖片。");var image=new BitmapImage();image.BeginInit();image.CacheOption=BitmapCacheOption.OnLoad;image.StreamSource=stream;image.EndInit();int stride=(image.PixelWidth*image.Format.BitsPerPixel+7)/8;var pixels=new byte[stride*image.PixelHeight];image.CopyPixels(pixels,stride,0);var normalized=BitmapSource.Create(image.PixelWidth,image.PixelHeight,96,96,image.Format,image.Palette,pixels,stride);normalized.Freeze();return normalized;}
